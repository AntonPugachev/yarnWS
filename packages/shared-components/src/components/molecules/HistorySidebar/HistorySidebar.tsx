...
const HistorySidebar = ({
  ...
  const getActiveHistory = () => {
    ...
    const countHistory = history.length - 1;
    const indexByUserId = getIndexByUserId(history);
    const indexByApproveDate = getActiveIndex(history) !== -1 ? getActiveIndex(history) : countHistory;
    ...
    return history.reduce((acc: IHistory[], curr: IHistory, index: number) => {
      ...
      const nextTwoStep = acc.length < 4 && index > activeIndex;
  ...
  return (
    <div
      ...
      ref={containerRef}
    >
      <Tile variant={opened ? 'non-clickable' : 'none'} padding={opened ? '0' : '20px'} className='rf-history-sidebar__tile'>
        <div className={`rf-history-sidebar__list ${opened ? 'opened-padding' : ''}`} ref={listRef}>
          <div
            className={`rf-history-sidebar__head ${!opened ? 'rf-history-sidebar__head-closed' : 'rf-history-sidebar__head-opened'}`}
          >
            <Button
                    ...
                    opened && <div className='rf-history-sidebar__item-content'>
                      <h4 className='rf-history-sidebar__item-name'>
                        ...
                        {item.approvers.length > 1 &&
                          <Tooltip position='top' background='white' hidePadding>
