...
const getOffsetPadding = (offset: number, placement: TooltipPosition): CSSProperties => {
  const offsetPadding = {
    ...
    'bottom-start': { paddingTop: `${offset}px`, },
...
const TooltipContent: FC<ITooltipContentProps> = ({
  children, position = 'top', className = '', background = 'white', offset = 8, hidePadding,
    ...
    {({ ref, style: dropdownStyle, placement }) => (
      ...
      >
        <div className={`rf-tooltip__content rf-tooltip__content--${background} ${className}`}>
          <div
            className={
              `rf-tooltip__inner rf-tooltip__inner--${placement} 
            ${
      hidePadding ? 'rf-tooltip__inner--no-pudding' : ''}
            `}
          >
            {children}
          </div>
