...
const Tabs: React.FC<ITabsProps> = ({ list, showLine = true, showMenu = true, size = 'md', children }: ITabsProps) => {
  ...
  const lastVisibleIndexRef = useRef<number>(list.length - 1);
  ...
  const handleLastVisibleIndex = (width = 0) => {
    ...
    const MENU_WIDTH = 48;
    const hasHiddenIndex = lastVisibleIndexRef.current < list.length - 1;
    let visibleIndex = lastVisibleIndexRef.current;
  ...
  const isActiveIndexInMenu: boolean = active > lastVisibleIndexRef.current;
    ...
    return (
      <div
        ...
        className={
          classnames(
            'rf-tabs__link',
            i > lastVisibleIndexRef.current && 'rf-tabs__link--hidden'
          )
        }
        ref={refs.current[i]}
      >
            ...
            return [
              ...
              <div key={i} className={classnames('rf-tabs__link', 'rf-tabs__menu-link')}>
                ...
                >
                  {!!curr.icon && (
                    <div
                      className={classnames('rf-tabs__icon', 'rf-tabs__menu-icon')}>
                      {curr.icon}
                    </div>
                  )}
  ...
  const hasMenuNav = lastVisibleIndexRef.current < list.length - 1;
  ...
  return (
    <div className={'rf-tabs rf-tabs--buttons'} ref={ref}>
      <nav className={classnames('rf-tabs__navigation', showLine && 'rf-tabs__navigation__line')}>
        <div className={
          classnames('rf-tabs__navigation-list', !showMenu && 'rf-tabs__navigation-list--without-menu')}>
          {nav}
          {hasMenuNav && (
            <Menu
